                    ...
                    @if(Auth::check() && $user->id != Auth::id())
                        @if(! Auth::user()->isFriend($user->id))
                            @if(\App\FriendRequest::where('receiver', Auth::user()->id)->where('sender', $user->id)->exists())
                                <a href="{{ route('friend.addFriend', $user->id) }}"> <i class="fa fa-user-plus newtext"></i> </a>
                            @elseif(\App\FriendRequest::where('sender', Auth::user()->id)->where('receiver', $user->id)->exists())
                                <i class="fa fa-hourglass-end newtext"></i>
                            @else
                                <a href="{{ route('friend.addFriend', $user->id) }}"> <i class="fa fa-user-plus newtext"></i> </a>
                            @endif
                        @else
                            <i class="fa fa-check newtext"></i>
                        @endif
                    @endif
                </h2>
                                ...
                                @foreach($user->heroes as $hero)
                                    <div class="col-lg-12 hero">
                                        ...
                                        <div class="hero-content">
                                            <span class="name"><a href="{{ route('profile.hero.details', [$user->username, $hero->heroName]) }}">{{ $hero->heroName }}</a></span>
                                            <div class="info">
