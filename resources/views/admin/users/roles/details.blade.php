    ...
    <section id="main-content">
        ...
        <div class="row">
            <div class="small-16 columns newtext">
                <h1><i class="fa fa-plus-circle newtext"></i> Update Role</h1>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            @foreach($permission as $p)
                                                <div class="row"><input id="{{ camel_case($access . $p['access']) }}" type="checkbox" name="permissions[{{ strtolower($access . '.' . $p['access']) }}]" @if($role->hasPermission(strtolower($access . '.' . $p['access']))) checked @endif><label for="{{ camel_case($access . $p['access']) }}"><span data-tooltip aria-haspopup="true" class="has-tip" title="{{ $p['description'] }}">{{ preg_replace('/(?<!\ )[A-Z]/', ' $0', $p['access']) }}</span></label></div>
                                            @endforeach
                                        </td>
                                    </tr>
                                @endforeach
